...
BEGIN
    ...
    IF V_TASKSTATE<>'2'  THEN 
     p_retCode := 'S094570157'; p_retMsg  := '此订单的任务状态不为处理成功状态，不可删除任务';
          ROLLBACK;RETURN;
    END IF;
    ...
    EXCEPTION
       WHEN OTHERS THEN
            p_retCode := 'S094570110'; p_retMsg  := '记录制卡任务历史表失败' || SQLERRM;
            ROLLBACK; RETURN;                   
    END;
    ...
    BEGIN                        
        DELETE TD_XFC_INITCARD T WHERE T.XFCARDNO BETWEEN V_STARTCARDNO AND V_ENDCARDNO  ;
         IF  SQL%ROWCOUNT = 0 THEN RAISE V_EX; END IF;                                                
          EXCEPTION                                                
          WHEN OTHERS THEN                                                
             p_retCode := 'S094570164'; p_retMsg  := '删除充值卡帐号表数据失败' || SQLERRM;                                                
             ROLLBACK; RETURN; 
     END;                          
