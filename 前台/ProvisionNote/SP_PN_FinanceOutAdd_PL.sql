...
<DESCRIPTION>出金确认存储过程</DESCRIPTION>
------------------------------------ */
CREATE OR REPLACE PROCEDURE SP_PN_FINANCEOUTADD
(
...
BEGIN
	...
	END;
	--银行金额<业务金额
	IF ABS(V_BANKCOUNT) <= ABS(V_TRADECOUNT) THEN
		...
		FOR V_C IN (SELECT F2,F3 FROM TMP_COMMON WHERE F0 = P_SESSIONID AND F1 = '0' AND F5 IS NULL ORDER BY F2) LOOP
			...
			FOR	V_D IN (SELECT F2,F3,F4 FROM TMP_COMMON WHERE F0 = P_SESSIONID AND F1 = '1' AND F5 IS NULL ORDER BY F2) LOOP
				...
				IF(V_LEFTMONEY < 0) THEN
					--银行金额>业务未使用金额
					IF ABS(V_BANKMONEY) >= ABS(V_LEFTMONEY) THEN
						--此次匹配的金额
	...
	IF ABS(V_BANKCOUNT) > ABS(V_TRADECOUNT) THEN
		...
		FOR V_C IN (SELECT F2,F3 FROM TMP_COMMON WHERE F0 = P_SESSIONID AND F1 = '1' ORDER BY F2) LOOP
		
			--此次需要匹配的业务金额
			V_TRADEMONEY := V_C.F2;
			
			--循环业务
			FOR	V_D IN (SELECT F2,F3 FROM TMP_COMMON WHERE F0 = P_SESSIONID AND F1 = '0' ORDER BY F2) LOOP
				--GET TRADE ID
				SP_GETSEQ(SEQ => V_SEQ);
				--每次循环时，此次匹配金额重置为0
				V_RELMONEY := 0;
				...
				IF(V_LEFTMONEY < 0) THEN
					--业务金额>银行未使用金额
					IF ABS(V_TRADEMONEY) >= ABS(V_LEFTMONEY) THEN
						--此次匹配的金额
						V_RELMONEY := V_LEFTMONEY;
						--剩余未匹配的银行金额--用以循环业务表再次匹配
