...
CREATE OR REPLACE PROCEDURE SP_BFJ_CompletePay
(
...
BEGIN
	...
	BEGIN 
		...
		IF v_count = 1 THEN
			p_retCode := 'SBFJ003000';
			p_retMsg  := p_tradeDate || '已出金确认完成';
			ROLLBACK; RETURN;			
		END IF;   
	END;
	...
	BEGIN
		...
		AND ISINMATCH = '1';																	
		IF v_count != 1 THEN
			p_retCode := 'SBFJ003002';
			p_retMsg  := '请先确认完成上一日账务' || SQLERRM;
			ROLLBACK; RETURN;			
		END IF;  
	END; 
	END IF;  
	...
	BEGIN
		 ...
		 LOOP
			 IF v_cur.BANKACCOUNT IS NULL THEN
				...
				p_retMsg  := v_cur.SYSTEMNAME ||'没有入账，无法确认出金' || SQLERRM;
				ROLLBACK; RETURN;		
			 END IF;
			...
			BEGIN
				--计算业务未达账
				BEGIN
				SELECT ABS(NVL(SUM(NVL(LEFTMONEY,TRADECHARGE)),0)) INTO v_businessless																				
				FROM TF_F_BFJ_OCAB																				
				...
				--计算银行未达账
				BEGIN
				SELECT ABS(NVL(SUM(NVL(LEFTMONEY,TRADEMONEY)),0)) INTO v_bankless																				
	...
	--5.更新备付金任务表
	BEGIN
		UPDATE TF_F_BFJ_TASK SET ISOUTMATCH = '1' ,OUTMATCHSTAFFNO = p_currOper , OUTMATCHDATE = v_now,																																		
