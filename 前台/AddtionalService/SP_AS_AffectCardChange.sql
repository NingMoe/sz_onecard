CREATE OR REPLACE PROCEDURE SP_AS_AffectCardChange
(
    P_ID                CHAR,
    P_OLDCARDNO         CHAR,
    P_NEWCARDNO         CHAR,
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        P_RETCODE := 'S00506B001'; P_RETMSG  := '更新惠民亲子卡旧卡有效标志失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
        P_RETCODE := 'S00506B004'; P_RETMSG  := '从旧卡向新卡复制惠民亲子卡信息失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        VALUES
            (V_SEQNO,P_ID,'66',P_NEWCARDNO,P_ASN,V_CARDTYPE,P_OLDCARDNO,
            P_CURROPER,P_CURRDEPT,V_TODAY,P_PACKAGETYPECODE);
    EXCEPTION WHEN OTHERS THEN
        P_RETCODE := 'S00506B005'; P_RETMSG  := '新增惠民亲子卡补换卡台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        VALUES(V_SEQNO, '66', P_OPERCARDNO, P_NEWCARDNO, P_TERMINALNO, P_ENDDATENUM, V_TODAY);
    EXCEPTION WHEN OTHERS THEN
        P_RETCODE := 'S00506B006'; P_RETMSG  := '新增惠民亲子卡补换卡卡片交易台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        USING      DUAL
        ON         (T1.CARDNO = P_NEWCARDNO AND T1.FUNCTIONTYPE = '18')
        WHEN MATCHED THEN
        ...
        WHEN NOT MATCHED THEN
            ...
            VALUES  (P_NEWCARDNO, '18'        , '1'   , V_ENDDATE, P_CURROPER, V_TODAY , P_PACKAGETYPECODE);

        IF  SQL%ROWCOUNT != 1 THEN RAISE V_EX; END IF;

    EXCEPTION WHEN OTHERS THEN
        P_RETCODE := 'S00506B007'; P_RETMSG  := '新增卡片与惠民亲子卡功能项关联关系失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        SET    USETAG        = '0'       ,  ENDTIME       = NULL,
               UPDATESTAFFNO = P_CURROPER , UPDATETIME    = V_TODAY
        WHERE  CARDNO = P_OLDCARDNO AND FUNCTIONTYPE = '18';

        IF  SQL%ROWCOUNT != 1 THEN RAISE V_EX; END IF;
    EXCEPTION WHEN OTHERS THEN
        P_RETCODE := 'S00506B008'; P_RETMSG  := '关闭卡片与惠民亲子卡功能项关联关系失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
