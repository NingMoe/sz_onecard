CREATE OR REPLACE PROCEDURE SP_AS_GardenCardRenew
(
    p_ID                char,
    p_cardNo            char,
    p_cardTradeNo       char,
...
BEGIN
    ...
    begin
        ...
        if trunc(v_updateTime, 'DD') = trunc(sysdate, 'DD') then
            p_retCode := 'S00501B000'; p_retMsg  := '园林年卡当日开通，不允许再次续费';
            return;
        end if;
    ...
    BEGIN
        ...
        VALUES
            (v_seqNo,p_ID,'3H',p_cardNo,p_asn,v_cardType,
            p_currOper,p_currDept,v_today,substr(p_rsrv1,0,2));
    EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
        ...
        VALUES
            (p_ID,v_seqNo,'3H',p_cardNo,p_cardTradeNo,p_tradeFee,
    ...
    BEGIN
        ...
        VALUES(v_seqNo, '3H', p_operCardNo, p_cardNo, p_terminalNo, p_endDateNum, v_today);
    EXCEPTION WHEN OTHERS THEN
