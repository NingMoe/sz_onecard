CREATE OR REPLACE PROCEDURE SP_AS_RelaxCardAppend
(
    p_cardNo             char,
    p_asn                char,
...
BEGIN
    ...
    BEGIN
        ...
        VALUES
            (v_seqNo, '34',p_cardNo,p_asn,v_cardType,
            p_currOper,p_currDept,v_today,p_packageTypeCode);
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00507B001'; p_retMsg  := '新增惠民休闲年卡补写卡台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        VALUES(v_seqNo, '34', p_operCardNo, p_cardNo, p_terminalNo, p_endDateNum, v_today);
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00507B002'; p_retMsg  := '新增惠民休闲年卡补写卡卡片交易台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
