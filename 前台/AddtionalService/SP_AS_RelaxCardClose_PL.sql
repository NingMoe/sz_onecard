CREATE OR REPLACE PROCEDURE SP_AS_RelaxCardClose
(
    p_ID                char,
    p_cardNo            char,
    p_cardTradeNo       char,
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00508B001'; p_retMsg  := '设置休闲年卡有效标识为无效时失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        VALUES
            (v_seqNo,p_ID,'3B',p_cardNo,p_asn,v_cardType,
            p_currOper,p_currDept,v_today);
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00508B002'; p_retMsg  := '新增休闲年卡关闭台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        INSERT INTO TF_B_TRADEFEE
            ...
            OPERATESTAFFNO,OPERATEDEPARTID,OPERATETIME)
        VALUES
            (p_ID,v_seqNo,'3B',p_cardNo,p_cardTradeNo,p_tradeFee,
            p_currOper,p_currDept,v_today);
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00508B003'; p_retMsg  := '新增休闲年卡关闭现金台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        VALUES(v_seqNo, '3B', p_operCardNo, p_cardNo, p_terminalNo, p_endDateNum, v_today);
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00508B004'; p_retMsg  := '新增休闲年卡关闭卡片交易台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        IF  SQL%ROWCOUNT != 1 THEN RAISE v_ex; END IF;
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00508B005'; p_retMsg  := '更新卡片与休闲年卡功能项关联关系失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
