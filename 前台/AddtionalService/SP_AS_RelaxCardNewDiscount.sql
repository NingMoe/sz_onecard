CREATE OR REPLACE PROCEDURE SP_AS_RelaxCardNewDiscount
(
    p_ID                char,
    p_cardNo            char,
    p_cardTradeNo       char,
...
BEGIN
  ...
  END;
    ...
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00505B005'; p_retMsg  := '新增或者更新惠民休闲年卡信息失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    begin
        ...
        if trunc(v_updateTime, 'DD') = trunc(sysdate, 'DD') then
            p_retCode := 'S00501B000'; p_retMsg  := '惠民休闲年卡当日开通，不允许再次开通';
            return;
        end if;
    ...
    BEGIN
        ...
        VALUES
            (p_TradeID,p_ID,'25',p_cardNo,p_asn,v_cardType,
            p_currOper,p_currDept,v_today,p_XFCARDNO,p_PACKAGETPYECODE);
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00505B006'; p_retMsg  := '新增惠民休闲年卡开通台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        VALUES
            (p_ID,p_TradeID,'25',p_cardNo,p_cardTradeNo,v_tradefee,-p_discount,
            p_currOper,p_currDept,v_today);
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00505B009'; p_retMsg  := '新增惠民休闲年卡开通现金台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
    ...
    BEGIN
        ...
        VALUES(p_TradeID, '25', p_operCardNo, p_cardNo, p_terminalNo, p_endDateNum, v_today);
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00505B010'; p_retMsg  := '新增惠民休闲年卡开通卡片交易台帐失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
  ...
  END;
    ...
    BEGIN
        ...
        ON (CARDNO        = p_cardNo AND FUNCTIONTYPE  = '08')
        WHEN MATCHED THEN
            UPDATE
            SET    USETAG        = '1',
                   ENDTIME       = v_endDate ,
        ...
        WHEN NOT MATCHED THEN
            ...
            VALUES(p_cardNo, '08'        , '1'   , v_endDate, p_currOper    , v_today  , p_PACKAGETPYECODE  ,v_RSRV1,v_RSRV3);

        IF  SQL%ROWCOUNT != 1 THEN RAISE v_ex; END IF;
    EXCEPTION WHEN OTHERS THEN
        p_retCode := 'S00505B011'; p_retMsg  := '更新或新增卡片与惠民休闲年卡功能项关联关系失败,' || SQLERRM;
        ROLLBACK; RETURN;
    END;
