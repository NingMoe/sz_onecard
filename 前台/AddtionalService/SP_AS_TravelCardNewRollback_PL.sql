CREATE OR REPLACE PROCEDURE SP_AS_TravelCardNewRollback
(
    p_ID                char,
    p_cardNo            char,
    p_cardTradeNo       char,
...
BEGIN
    if p_option in ('0', '2') then
        ...
        loop
            ...
            if v_cur.TRADETYPECODE = '6A' then
    ...
    if p_cancelTradeId is null then
        raise_application_error(-20101, '没有当月可以返销的吴江旅游年卡开通交易' );
        ...
        begin
            ...
            from tf_b_trade
            where cardno = p_cardNo
            and tradetypecode = 'A6'
            and operatetime >= v_opTime;

            raise_application_error(-20101, '已经返销过吴江旅游年卡开通，不允许递归返销' );
    ...
    if p_option in ('0', '1') then
        -- 2) Update
        SELECT NVL(RERVCHAR, 'FFFFFFFFFFFF') INTO p_endDateNum
        ...
        IF  SQL%ROWCOUNT != 1 THEN
            raise_application_error(-20103, '设置吴江旅游年卡有效标识为无效时失败' );
        END IF;
        ...
        SET    USETAG        = DECODE(p_endDateNum, 'FFFFFFFFFFFF', '0', '1'),
               UPDATESTAFFNO = p_currOper,
               UPDATETIME    = v_today,
               ENDTIME       = substr(p_endDateNum, 3, 8)
        WHERE  CARDNO        = p_cardNo
        AND    FUNCTIONTYPE  = '12';
        IF  SQL%ROWCOUNT != 1 THEN
            raise_application_error(-20104, '更新卡片与吴江旅游年卡功能项关联关系失败' );
        END IF;
