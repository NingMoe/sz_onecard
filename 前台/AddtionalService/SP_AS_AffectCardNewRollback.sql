...
BEGIN
    ...
    if p_cancelTradeId is null then
        raise_application_error(-20101, '没有当月可以返销的亲子卡开通交易' );
    else
        ...
        begin
            ...
            raise_application_error(-20101, '已经返销过亲子卡开通，不允许递归返销' );
    ...
    if p_option in ('0', '1') then
        ...
        IF  SQL%ROWCOUNT != 1 THEN
            raise_application_error(-20103, '设置惠民亲子卡有效标识为无效时失败' );
        END IF;
        ...
        AND    FUNCTIONTYPE  = '18';
        IF  SQL%ROWCOUNT != 1 THEN
            raise_application_error(-20104, '更新卡片与惠民亲子卡功能项关联关系失败' );
        END IF;
