CREATE OR REPLACE PROCEDURE SP_GC_Open
(
...
BEGIN
    ...
    BEGIN
        FOR v_cur in (SELECT f1 FROM TMP_COMMON)
        LOOP
    ...
    AND  CARDNO IN (SELECT f1 FROM TMP_COMMON);
    ...
    IF p_oldFlag = '0' THEN
        ...
        FROM   TF_F_CARDREC c
        WHERE  c.CARDNO IN (SELECT f1 FROM TMP_COMMON);
        ...
        AND    t.CARDNO IN (SELECT f1 FROM TMP_COMMON);
    ...
    EXCEPTION
        WHEN OTHERS THEN
            p_retCode := 'S004P01B05'; p_retMsg  := '新增企服卡开卡总量台帐,' || SQLERRM;
            ROLLBACK; RETURN;
    END;
