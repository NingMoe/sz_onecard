CREATE OR REPLACE PROCEDURE SP_UC_ResStateChange
(
...
BEGIN
    ...
    LOOP
	    IF cur_renewdata.f1 is null THEN
		    UPDATE TL_R_ICUSER SET resstatecode = '00' WHERE cardno = cur_renewdata.f0 AND RESSTATECODE = '01';
		    
		    IF  SQL%ROWCOUNT != 1 THEN
                raise_application_error(-20101, '¸üÐÂ×´Ì¬Ê§°Ü' );
		    END IF;
		ELSE
			SELECT COUNT(*) INTO v_amount FROM TL_R_ICUSER 
		    WHERE CARDNO BETWEEN cur_renewdata.f0 and cur_renewdata.f1;

		    UPDATE TL_R_ICUSER SET resstatecode = '00' 
		    WHERE CARDNO BETWEEN cur_renewdata.f0 and cur_renewdata.f1 AND RESSTATECODE = '01';
            ...
            VALUES
            (v_TradeID, '14', cur_renewdata.f0, cur_renewdata.f1, p_currOper, p_currDept, p_CURRENTTIME);
    	END IF;			
	END LOOP;
