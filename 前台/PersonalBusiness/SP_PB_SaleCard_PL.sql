CREATE OR REPLACE PROCEDURE SP_PB_SaleCard
(
		p_ID	            char,
		p_CARDNO	        char,
		p_DEPOSIT	        int,
...
BEGIN
    ...
    END;
     ...
     -- 12) Log the sync information while CARD_TYPE is 61
  BEGIN
  IF SUBSTR(p_CARDNO,0,6)='215061' THEN
    INSERT INTO TF_B_SYNC(TRADEID,CITIZEN_CARD_NO,Trans_Code,Name,Paper_Type,Paper_No,SEX,Card_Type,OPERATESTAFFNO,OPERATEDEPARTNO,OPERATETIME)
    VALUES(v_TradeID,p_CARDNO,'9501',p_CUSTNAME,p_PAPERTYPECODE,p_PAPERNO,p_CUSTSEX,'61',p_currOper,p_currDept,p_CURRENTTIME);
  END IF;
    EXCEPTION
          WHEN OTHERS THEN
