create or replace procedure SP_WA_LossBlackList
(
    p_funcCode     varchar2,
    p_oldCardNo    char    ,
    p_cardNo       char    ,
...
begin
    ...
    if p_funcCode = 'Add' then
        insert into TF_B_LOSS_BLACK(CARDNO, CREATETIME, WARNTYPE, 
    ...
    -- 生成备份台账
    insert into TH_B_LOSS_BLACK(HSEQNO, BACKTIME, BACKSTAFF, 
        BACKWHY, 
    ...
    select TH_B_WARN_BLACK_SEQ.nextval, v_today, p_currOper, 
        ...
        UPDATETIME, DOWNTIME, REMARK 
    from TF_B_LOSS_BLACK
    where CARDNO = p_cardNo;

    -- 修改黑名单
    if p_funcCode = 'Mod' then
        update TF_B_LOSS_BLACK
