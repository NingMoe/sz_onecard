...
namespace ChargeCardDataProduce
{
    ...
    public abstract class OracleHelper
    {
        ...
        public static  string CONNECTION_STRING =Validator.DecryptDES(System.Configuration.ConfigurationSettings.AppSettings["OraConnString1"],"szcic201");
        //public static string CONNECTION_STRING1 = Validator.EncryptDES("Data Source=SZTTEST;Persist Security Info=True;Password=uowner;User ID=uowner_b_sz;Max Pool Size=100;Min Pool Size=5;Pooling=true", "szcic201");
        //public static readonly string CONNECTION_STRING =LogisticsConfiguration.ConnectionString;
        ...
        /// <summary>
        /// 这个方法将dataRow中的值赋给参数对象数组
        /// </summary>
        /// <param name="commandParameters">要赋值的参数对象数组</param>
        /// <param name="dataRow">要赋给参数对象的DataRow值</param>
        ...
        /// 这个方法将数组中的值赋给参数对象数组
        /// </summary>
        /// <param name="commandParameters">需要指派值的参数数组</param>
        /// <param name="parameterValues">要指派给参数数组的数组值</param>
        private static void AssignParameterValues(OracleParameter[] commandParameters, object[] parameterValues)
        {
            ...
            {
                // 如果当前数组的值来源为（IDbDataParameter）,那么赋（IDbDataParameter）类型的值
        ...
        /// <summary>
        /// 这个方法分配连接、事务、命令类型、参数给命令对象
        /// </summary>
        /// <param name="command">将要使用的命令对象</param>
        /// <param name="connection">命令对象使用的有效连接对象</param>
        /// <param name="transaction">有效的事务对象,可以为 'null'</param>
        /// <param name="commandType">解释命令字符串的类型CommandType(stored procedure, text, etc.)</param>
        /// <param name="commandText">存储过程名称或者T-Oracle语句</param>
        /// <param name="commandParameters">参数数组，执行命令的参数，如果不是必须可以为（null）</param>
        /// <param name="mustCloseConnection"><c>true</c> 进入方法后如果连接是打开的，其值设为false.</param>
        ...
        /// <summary>
        /// 根据数据库连接字符串执行一个命令(执行命令没有参数，并且不返回结果集)
        /// </summary>
        /// <remarks>
        /// e.g.:  
        ///  int result = ExecuteNonQuery(connString, CommandType.StoredProcedure, "PublishOrders");
        /// </remarks>
        /// <param name="connectionString">一个有效的OracleConnection对象的连接字符串</param>
        /// <param name="commandType">解释命令字符串的类型CommandType(stored procedure, text, etc.)</param>
        /// <param name="commandText">存储过程名称或者T-Oracle语句</param>
        ...
        /// <summary>
        /// 根据数据库连接字符串及参数执行一个命令(执行命令不返回结果集)
        /// </summary>
        /// <remarks>
        /// e.g.:  
        ...
        /// <summary>
        /// 根据数据库连接字符串及参数执行一个存储过程命令(执行命令不返回结果集)
        /// </summary>
        /// <remarks>
        /// e.g.:  
        ...
        public static int RunExecuteNonQueryStoredProcedure(String connectionString, String spName, OracleParameter[] commandParameters)
        {
