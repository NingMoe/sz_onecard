...
public class HttpHelper
{
    ...
    private static readonly string _zzOrderQueryUrl = ConfigurationManager.AppSettings["ZZOrderQueryUrl"];
    ...
    public static string PostRealRequest(TradeType tradeType, Dictionary<string, string> postData, TokenType tokenType)
    {
        ...
        else if (tradeType == TradeType.ZZOrderQuery)
        {
            url = _zzOrderQueryUrl;
        }
    ...
    {
        return PostRealRequest(tradeType, postData, TokenType.ZZToken);
    }
    ...
    {
        dataDictionary.Add("CHANNELKEY", key);
        //拼接字符串
        string sign = "";
