...
public partial class ASP_PartnerShip_PS_DeptBalFiApproval : Master.Master
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            lvwBalUnitsAppral.DataKeyNames =
                new string[]{"DTRADETYPECODE","TRADEID","REMARK","BANKCODE","FINBANKCODE","DEPTTYPECODE","BALCYCLETYPECODE",
               "FINCYCLETYPECODE","FINTYPECODE","BIZTYPE","DBALUNITNO","DBALUNIT",  
    ...
    {
        DataTable data = SPHelper.callQuery("SP_PS_Query_DeptBal", context, "QueryDeptBalFiApprovalInfo");
    ...
    public void lvwBalUnitsAppral_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        hidBankCode.Value = getDataKeys("BANKCODE");
        hidFinBankCode.Value = getDataKeys("FINBANKCODE");
        hidDeptTypeCode.Value = getDataKeys("DEPTTYPECODE");
        hidBalCycleTypeCode.Value = getDataKeys("BALCYCLETYPECODE");
        hidFinCycleTypeCode.Value = getDataKeys("FINCYCLETYPECODE");
        hidFinTypeCode.Value = getDataKeys("FINTYPECODE");
    ...
    protected void btnPass_Click(object sender, EventArgs e)
    {
        ...
        context.AddField("P_DBALUNIT").Value = labBalUnit.Text.Trim();
        context.AddField("P_BANKCODE").Value = hidBankCode.Value;
        context.AddField("P_BANKACCNO").Value = labBankAccNo.Text.Trim();
        context.AddField("P_BALCYCLETYPECODE").Value = hidBalCycleTypeCode.Value;
        context.AddField("P_BALINTERVAL").Value = labBalInterval.Text.Trim();
        context.AddField("P_FINCYCLETYPECODE").Value = hidFinCycleTypeCode.Value;
        context.AddField("P_FININTERVAL").Value = labFinCyclInterval.Text.Trim();
        context.AddField("P_FINTYPECODE").Value = hidFinTypeCode.Value;
        context.AddField("P_FINBANKCODE").Value = hidFinBankCode.Value;
        context.AddField("P_LINKMAN").Value = labLinkMan.Text.Trim();
        context.AddField("P_UNITPHONE").Value = labPhone.Text.Trim();
        context.AddField("P_UNITADD").Value = labAddress.Text.Trim();
        context.AddField("P_UNITEMAIL").Value = labEmail.Text.Trim();
        context.AddField("P_DEPTTYPE").Value = hidDeptTypeCode.Value;
        context.AddField("P_PREPAYWARNLINE").Value = Convert.ToInt32(labPrepayWarnLine.Text.Trim())*100;
        context.AddField("P_PREPAYLIMITLINE").Value = Convert.ToInt32(labPrepayLimitLine.Text.Trim()) * 100;
        context.AddField("P_REMARK").Value = labReMark.Text.Trim();
        bool ok = context.ExecuteSP("SP_PS_DeptBalFiApp_Pass");
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        ...
        context.SPOpen();
        context.AddField("P_TRADEID").Value = hidTradeID.Value;
        bool ok = context.ExecuteSP("SP_PS_DeptBalFiApp_Cancel");
    ...
    private void ClearBalUnit()
    {
        ...
        hidBankCode.Value = "";
        hidFinBankCode.Value = "";
        hidDeptTypeCode.Value = "";
        hidBalCycleTypeCode.Value = "";
        hidFinCycleTypeCode.Value = "";
        hidFinTypeCode.Value = "";
