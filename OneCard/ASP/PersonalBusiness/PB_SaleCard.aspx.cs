...
public partial class ASP_PersonalBusiness_PB_SaleCard : Master.FrontMaster
{
        ...
        else if (hidpdotype.Value == "02")
        {
            ...
            else if (hidWarning.Value == "writeSuccess")
            {
                ...
                if (ok)
                {
                    ...
                    #region add by liuhe 20111231 添加对代理营业厅预付款的验证,扣费后如果超过预警额度则提示
                    int opMoney = Convert.ToInt32(Convert.ToDecimal(SupplyMoney.Text) * 100) + Convert.ToInt32(Convert.ToDecimal(SupplyOtherFee.Text) * 100);
                    DeptBalunitHelper.ValdatePrepay(context, opMoney, "1");
                    #endregion 
                }
            }
    ...
    protected void btnSale_Click(object sender, EventArgs e)
    {
        ...
        if (ok)
        {
            ...
            {
                string tradeID = ((SP_PB_SaleCard_COMMITPDO)pdoOut).TRADEID;
    ...
    protected void btnSupply_Click(object sender, EventArgs e)
    {
        ...
        if (ok)
        {
            ...
            //add by youyue 20140718 充值热敏打印凭证数据准备
            ASHelper.preparePingZheng(PrintRMPingZheng, txtCardno.Text, txtCusname.Text, "充值", SupplyMoney.Text
                , "", "", txtCustpaperno.Text, (Convert.ToDecimal(pdo.CARDMONEY + pdo.SUPPLYMONEY) / 100).ToString("0.00"),
                "", SupplyMoney.Text, context.s_UserID, context.s_DepartName, selPapertype.Text, "0.00", "");
