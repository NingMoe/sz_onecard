...
public partial class ASP_PersonalBusiness_RegisteredPeriodExtend : Master.FrontMaster
{
    ...
    protected void btnReadCardProcess()
    {
        ...
        if (ok)
        {
            ...
            if (string.IsNullOrEmpty(ddoTF_F_CUSTOMERRECOut.PAPERNO) || string.IsNullOrEmpty(ddoTF_F_CUSTOMERRECOut.CUSTNAME))//表示此卡是不记名卡
            {
                context.AddError("此卡是不记名卡，请确认！");
                return;
            }
    ...
    protected void btnSupply_Click(object sender, EventArgs e)
    {
        ...
        string availableDateStr = "99991231";
    ...
    {
        ASHelper.preparePingZheng(ptnPingZheng, hiddentxtCardno.Value, CustName.Text,
