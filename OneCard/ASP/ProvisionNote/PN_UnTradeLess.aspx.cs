...
public partial class ASP_ProvisionNote_PN_UnTradeLess : Master.Master
{
    // 页面装载
    ...
    protected void btnQuery_Click(object sender, EventArgs e)
    {
        ...
        DataTable data = PNHelper.callQuery(context, "QUERYUNTRADELESS", txtFromDate.Text, txtToDate.Text);
    ...
    protected void btnTrade_Click(object sender, EventArgs e)
    {
        ...
        bool ok = context.ExecuteSP("SP_PN_UNTRADEMAY");

        if (ok)
        {
            context.AddMessage("不匹配业务匹配成功");
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        context.SPOpen();
        context.AddField("P_SESSIONID").Value = Session.SessionID;
        bool ok = context.ExecuteSP("SP_PN_UNTRADELESS");

        if (ok)
        {
            context.AddMessage("不匹配业务取消成功");
            btnQuery_Click(sender, e);
        }
            ...
            if (cb != null && cb.Checked)
            {
                ...
                if (Convert.ToDecimal(gvr.Cells[2].Text) * 100 > 0)
                {
                    ...
                    context.ExecuteNonQuery("insert into TMP_COMMON (F0,F1,F2,F3) values('" +
                         Session.SessionID + "', '0','" + Convert.ToDecimal(gvr.Cells[2].Text) * 100 + "','" + gvr.Cells[11].Text + "')");
                ...
                else
                {
                    //负数银行帐务录入临时表
                    context.ExecuteNonQuery("insert into TMP_COMMON (F0,F1,F2,F3) values('" +
                         Session.SessionID + "', '1','" + Convert.ToDecimal(gvr.Cells[2].Text) * 100 + "','" + gvr.Cells[11].Text + "')");
    ...
    protected void gvBank_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            e.Row.Cells[11].Visible = false;    //TRADEID隐藏
        }
    }
    ...
    protected void btnHide_Click(object sender, EventArgs e)
    {
        ...
        context.SPOpen();
        context.AddField("P_SESSIONID").Value = Session.SessionID;
        bool ok = context.ExecuteSP("SP_PN_UnTradeHide");
