 ...
 * 2012/12/7   董翔			初次开发
 ****************************************************************/
public partial class ASP_ResourceManage_RM_ContractManager : Master.Master
{
    //初始化
    ...
    protected void btnQuery_Click(object sender, EventArgs e)
    {
        ...
        gvResult.DataSource = QueryContractList();
        gvResult.DataBind();
    ...
    protected Boolean submitValidation()
    {
        ...
        if (string.IsNullOrEmpty(txtContractName.Text.Trim()))
            context.AddError("合同名称不能为空", txtContractName);
        else if (Validation.strLen(txtContractName.Text.Trim()) > 50)
            context.AddError("合同名称长度不能超过50位", txtContractName);

        //合同编号
        if (string.IsNullOrEmpty(txtContractID.Text.Trim()))
            context.AddError("合同编号不能为空", txtContractID);
        else if (Validation.strLen(txtContractID.Text.Trim()) > 50)
            context.AddError("合同编号长度不能超过50位", txtContractID);

        //签订单位
        if (string.IsNullOrEmpty(txtSigningCompany.Text.Trim()))
            context.AddError("签订单位不能为空", txtSigningCompany);
        else if (Validation.strLen(txtSigningCompany.Text.Trim()) > 50)
            context.AddError("签订单位度不能超过50位", txtSigningCompany);

        //签订日期
        if (string.IsNullOrEmpty(txtSigningDate.Text.Trim()))
            context.AddError("签订日期不能为空", txtSigningDate);
        else if (!Validation.isDate(txtSigningDate.Text.Trim(), "yyyyMMdd"))
            context.AddError("签订日期格式必须为yyyyMMdd", txtSigningDate);

        //备注
        if (!string.IsNullOrEmpty(txtReMark.Text.Trim()))
            if (Validation.strLen(txtReMark.Text.Trim()) > 255)
                context.AddError("合同内容概要长度不能超过255位", txtReMark);
        ...
        if (FileUpload1.HasFile)
        {
            ...
            {
                context.AddError("合同扫描件大小不能大于5兆", FileUpload1);
            }
            if (FileUpload1.FileName.Length > 50)
            {
                context.AddError("合同扫描件文件名长度不能大于50位", FileUpload1);
            }
        }
    ...
    /// </summary>
    /// <returns></returns>
    protected ICollection QueryContractList()
    {
        ...
        DataTable data = SPHelper.callQuery("SP_RM_OTHER_Query", context, "Query_ContractList", parm);
    ...
    //查询合同明细
    public void QueryContractInfo(string contractid)
    {
    ...
    protected void btnContractAdd_Click(object sender, EventArgs e)
    {
        ...
        context.AddField("P_DOCUMENTTYPE").Value = "0";
    ...
    //修改合同
    protected void btnContractModify_Click(object sender, EventArgs e)
    {
        //提交校验
        if (!submitValidation())
            return;
