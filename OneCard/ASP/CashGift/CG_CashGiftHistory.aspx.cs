...
public partial class ASP_CashGift_CG_CashGiftHistory : Master.Master
{
    ...
    protected void Init_Page()
    {
        ...
        FIHelper.selectDept(context, selOriginalDept, true);
        FIHelper.selectDept(context, selNowDept, true);
    ...
    private void InitCashCardSurfaceName()
    {
        ...
        GroupCardHelper.fill(selCardType, table, true);
    }
    /// <summary>
    /// 初始化列表
    ...
    {
        DataTable data = SPHelper.callQuery("SP_CG_Query", context, "CashGiftHistory", selOriginalDept.SelectedValue, selOriginalStaffs.SelectedValue, selNowDept.SelectedValue, selNowStaffs.SelectedValue,
            txtFromDate.Text.Trim(), txtToDate.Text.Trim(), ddlOperater.SelectedValue,selCardType.SelectedValue,context.s_DepartID);
    ...
    public void gvResult_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (row != null)
        {
            DataTable dt = SPHelper.callQuery("SP_CG_Query", context, "CashGiftHistoryDetail", row.Cells[0].Text.Trim().Substring(0, 4), row.Cells[1].Text.Trim().Substring(0, 4), row.Cells[2].Text.Trim().Substring(0, 6), row.Cells[3].Text.Trim().Substring(0, 4), row.Cells[4].Text.Trim().Substring(0, 6));
    ...
    protected void gvResult_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            operCount += Convert.ToInt32(GetTableCellValue(e.Row.Cells[5]));;
        }
        else if (e.Row.RowType == DataControlRowType.Footer)  //页脚 
        {
            e.Row.Cells[0].Text = "总计";
            e.Row.Cells[5].Text = operCount.ToString();
        }
    }
