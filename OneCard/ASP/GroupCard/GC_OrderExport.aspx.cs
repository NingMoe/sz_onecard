 ...
 * 初次编写
 * ********************************/
public partial class ASP_GroupCard_GC_OrderExport : Master.Master
{
    ...
    protected void addOrderNode(XmlDocument xmldoc, XmlNode ORDERS, string orderno, string rownum)
    {
        ...
        if (data != null && data.Rows.Count > 0)
        {
            ...
            if (data.Rows[0]["ORDERTYPE"].ToString() == "1")//单位订单
            {
                queryCustInfo = @"Select a.COMPANYPAPERTYPE,a.COMPANYPAPERNO,b.PAPERTYPE,b.ADDRESS,b.EMAIL,b.OUTBANK,b.OUTACCT,
                                         a.COMPANYMANAGERNO,a.COMPANYENDTIME
                ...
                if (queryCustInfodata.Rows.Count > 0)
                {
                    ...
                    XmlElement CORPIDNO = xmldoc.CreateElement("CORPIDNO");
                    CORPIDNO.InnerText = queryCustInfodata.Rows[0]["COMPANYMANAGERNO"].ToString();
                    ORDER.AppendChild(CORPIDNO);

                    string sql = "select count(*) From TD_M_BUYCARDCOMINFO a,TF_F_COMBUYCARDREG b Where a.COMPANYNO=b.COMPANYNO "+
                                  "And a.COMPANYPAPERMSG is not null And b.REMARK = '" + orderno + "'";
                    context.DBOpen("Select");
                    DataTable blobData = context.ExecuteReader(sql);
                    string hasCorpPaperInfo = blobData.Rows.Count > 0 ? "1" : "0";
                    
                    XmlElement CORPPAPERINFO = xmldoc.CreateElement("CORPPAPERINFO");
                    CORPPAPERINFO.InnerText = hasCorpPaperInfo;
                    ORDER.AppendChild(CORPPAPERINFO);

                    XmlElement CORPPAPERINDATE = xmldoc.CreateElement("CORPPAPERINDATE");
                    CORPPAPERINDATE.InnerText = queryCustInfodata.Rows[0]["COMPANYENDTIME"].ToString();
                    ORDER.AppendChild(CORPPAPERINDATE);
                }
            }
            else//个人订单
            {
            ...
            TRANSACTOR.InnerText = data.Rows[0]["TRANSACTOR"].ToString().Substring(0,6);
        ...
        data = GroupCardHelper.callOrderQuery(context, "ReaderOrderInfo", orderno);
        if (data != null && data.Rows.Count > 0)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                XmlNode READER = xmldoc.CreateElement("READER");
                ORDER.AppendChild(READER);

                XmlElement READERCOUNT = xmldoc.CreateElement("COUNT");
                READERCOUNT.InnerText = data.Rows[i]["COUNT"].ToString();
                READER.AppendChild(READERCOUNT);

                XmlElement READERPRICE = xmldoc.CreateElement("PRICE");
                READERPRICE.InnerText = data.Rows[i]["VALUE"].ToString();
                READER.AppendChild(READERPRICE);

                XmlElement READERSUM = xmldoc.CreateElement("TOTALMONEY");
                READERSUM.InnerText = data.Rows[i]["SUM"].ToString();
                READER.AppendChild(READERSUM);
            }
        }
        //获取园林年卡信息
        data = GroupCardHelper.callOrderQuery(context, "GardenCardOrderInfo", orderno);
        if (data != null && data.Rows.Count > 0)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                XmlNode GARDENCARD = xmldoc.CreateElement("GARDENCARD");
                ORDER.AppendChild(GARDENCARD);

                XmlElement GARDENCARDCOUNT = xmldoc.CreateElement("COUNT");
                GARDENCARDCOUNT.InnerText = data.Rows[i]["COUNT"].ToString();
                GARDENCARD.AppendChild(GARDENCARDCOUNT);

                XmlElement GARDENCARDPRICE = xmldoc.CreateElement("PRICE");
                GARDENCARDPRICE.InnerText = data.Rows[i]["VALUE"].ToString();
                GARDENCARD.AppendChild(GARDENCARDPRICE);

                XmlElement GARDENCARDSUM = xmldoc.CreateElement("TOTALMONEY");
                GARDENCARDSUM.InnerText = data.Rows[i]["SUM"].ToString();
                GARDENCARD.AppendChild(GARDENCARDSUM);
            }
        }
        //获取园林年卡信息
        data = GroupCardHelper.callOrderQuery(context, "RelaxCardOrderInfo", orderno);
        if (data != null && data.Rows.Count > 0)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                XmlNode RELAXCARD = xmldoc.CreateElement("RELAXCARD");
                ORDER.AppendChild(RELAXCARD);

                XmlElement RELAXCARDCOUNT = xmldoc.CreateElement("COUNT");
                RELAXCARDCOUNT.InnerText = data.Rows[i]["COUNT"].ToString();
                RELAXCARD.AppendChild(RELAXCARDCOUNT);

                XmlElement RELAXCARDPRICE = xmldoc.CreateElement("PRICE");
                RELAXCARDPRICE.InnerText = data.Rows[i]["VALUE"].ToString();
                RELAXCARD.AppendChild(RELAXCARDPRICE);

                XmlElement RELAXCARDSUM = xmldoc.CreateElement("TOTALMONEY");
                RELAXCARDSUM.InnerText = data.Rows[i]["SUM"].ToString();
                RELAXCARD.AppendChild(RELAXCARDSUM);
            }
        }
        //获取开票信息
        ...
        if (data != null && data.Rows.Count > 0)
        {
            for (int i = 0; i < data.Rows.Count; i++)
            {
                ...
                INVOICEMONEY.InnerText = Convert.ToInt32((Convert.ToDecimal(data.Rows[i]["INVOICEMONEY"].ToString()) * 100)).ToString();
                //(Convert.ToDecimal(data.Rows[i]["INVOICEMONEY"].ToString()) * 100).ToString()
