...
public partial class Logon : Master.MasterBase
{
                    ...
                    if (dr["SysName"].ToString() == Request.QueryString["SysName"] && (Boolean)dr["IsAuth"])
                    {
                        ...
                        if (ok)
                        {
                            Response.Redirect(string.Format("{0}?staff={1}&token={2}&type={3}&link={4}", dr["LoginURL"].ToString()
                                , Session["STAFF"], token, "normal", Request.QueryString["link"]));
                        }
                        else
                        {
    ...
    protected void LogonBtn_Click(object sender, EventArgs e)
    {
            ...
            strIpAdr,Response,Session, ref loginURL, ref staffTable, ref dt))
        {
            ...
            if (ok)
            {
                ...
                {
                    Response.Redirect(string.Format("{0}?staff={1}&token={2}&type={3}&power={4}", loginURL, Session["STAFF"], token, "normal", "one"));
                }
                else
                {
                    Response.Redirect(string.Format("{0}?staff={1}&token={2}&type={3}", loginURL, Session["STAFF"], token, "normal"));
                }
            }
            else
            {
